...
echo 950715 | sudo -S vgs tv_lvm_vg 2>/dev/null | grep -q tv_lvm_vg && \
    echo 950715 | sudo -S vgreduce --removemissing tv_lvm_vg --force 2>/dev/null || true

# Re-create PVs on nvme0n1 and sdb (overwritten by TieredVol carving)
echo 950715 | sudo -S pvcreate -f /dev/nvme0n1 2>&1
echo 950715 | sudo -S pvcreate -f /dev/sdb 2>&1

# Extend VG
echo 950715 | sudo -S vgextend tv_lvm_vg /dev/nvme0n1 /dev/sdb 2>&1

echo "LVM state after restore:"
echo 950715 | sudo -S vgs 2>/dev/null
echo 950715 | sudo -S pvs 2>/dev/null

# Verify LVM works
echo 950715 | sudo -S lvcreate -L 1G -n lvm_verify tv_lvm_vg 2>&1
LVV=$(echo 950715 | sudo -S lvs --noheadings -o lv_path tv_lvm_vg/lvm_verify 2>&1 | tr -d ' ')
echo "Verification LV: $LVV"
echo 950715 | sudo -S ./tiered_io --path "$LVV" --bench --size 64MB --raw 2>&1 | grep "Throughput"
echo 950715 | sudo -S lvremove -f tv_lvm_vg/lvm_verify 2>&1
...
for vol in $(echo 950715 | sudo -S ./tiered_setup --list 2>/dev/null | grep "tv_\|chunk" | awk '{print $1}' 2>/dev/null); do

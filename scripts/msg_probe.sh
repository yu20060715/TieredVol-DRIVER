#!/bin/bash
# msg_probe.sh — P3: dmsetup message / sysfs 專項 probe（重建版 2026-08-12）
...
step "sysfs: stale_ms"
v=$(cat /sys/kernel/tieredvol/stale_ms 2>&1) && pass "stale_ms=$v" || fail "stale_ms=$v"
step "sysfs: wear_bias"
v=$(cat /sys/kernel/tieredvol/wear_bias 2>&1) && pass "wear_bias=$v" || fail "wear_bias=$v"
step "sysfs: ema_shift"
v=$(cat /sys/kernel/tieredvol/ema_shift 2>&1) && pass "ema_shift=$v" || fail "ema_shift=$v"
step "sysfs: loglevel"
v=$(cat /sys/kernel/tieredvol/loglevel 2>&1) && pass "loglevel=$v" || fail "loglevel=$v"

echo "==== P3-2: policy / wear message ===="
step "msg adaptive_on"
MSG 0 "adaptive_on" && pass "adaptive_on" || fail "adaptive_on"
step "msg show_adaptive"
MSG 0 "show_adaptive" && pass "show_adaptive" || fail "show_adaptive"
step "msg adaptive_off"
MSG 0 "adaptive_off" && pass "adaptive_off" || fail "adaptive_off"
step "msg set_policy"
MSG 0 "set_policy adaptive" && pass "set_policy" || fail "set_policy"
step "msg set_stale_ms"
MSG 0 "set_stale_ms 3000" && pass "set_stale_ms" || fail "set_stale_ms"
step "msg set_wear_bias"
MSG 0 "set_wear_bias 1" && pass "set_wear_bias" || fail "set_wear_bias"
step "msg set_ema_shift"
MSG 0 "set_ema_shift 2" && pass "set_ema_shift" || fail "set_ema_shift"
step "msg reset_wear"
MSG 0 "reset_wear" && pass "reset_wear" || fail "reset_wear"
step "msg show_wear"
MSG 0 "show_wear" && pass "show_wear" || fail "show_wear"

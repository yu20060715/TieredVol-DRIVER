...
| `src/tiered_io.c` | CLI I/O 工具：info/read/write/bench/bench-all/BLKDISCARD，呼叫 tv_write/tv_read 驗證 scheduler | 431 |
...
`tiered_io` CLI 工具已實作，提供 `--read` / `--write` / `--bench` / `--bench-all` / `--info` 五種模式，可驗證 scheduler 的 I/O 路徑。

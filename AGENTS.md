...
| 檔案 | 職責 | 行數 |
|------|------|------|
| `src/tiered_sched.h` | 所有 struct（TV_DISK, TV_SEGMENT, TV_METADATA, TV_MAP, TV_BUFFER, TV_SCHED）+ API 宣告 | 80 |
| `src/tiered_sched.c` | Scheduler 核心：init、write（buffer + flush）、read（mapping + io_uring）、destroy | 217 |
| `src/tiered_mapper.c` | Logical ↔ Physical offset mapping（prefix sum + linear scan） | 41 |
| `src/tiered_stripe_buf.c` | Stripe buffer 管理（aligned_alloc ring buffer，滿了就 flush） | 32 |
...
| `src/tiered_io.c` | CLI I/O 工具：info/read/write/bench，呼叫 tv_write/tv_read 驗證 scheduler | 322 |
...
# tests/test_tui.c — TUI 解析測試（22 cases）：parse_bench_output, bench_disk_done

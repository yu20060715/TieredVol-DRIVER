...
| 檔案 | 職責 | 行數 |
|------|------|------|
| `src/tiered_sched.h` | 所有 struct（TV_DISK, TV_SEGMENT, TV_METADATA, TV_MAP, TV_SCHED, TV_STRIPE_BUF）+ API 宣告 | 78 |
| `src/tiered_sched.c` | Scheduler 核心：init、write（buffer + flush）、read（mapping + io_uring）、destroy | 241 |
| `src/tiered_mapper.c` | Logical ↔ Physical offset mapping（prefix sum + linear scan） | 41 |
| `src/tiered_io_uring.c` | io_uring wrapper（SQE/CQE、submit、wait） | 57 |
...
| `src/tiered_io.c` | CLI I/O 工具：info/read/write/bench，呼叫 tv_write/tv_read 驗證 scheduler | 376 |

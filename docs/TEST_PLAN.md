...
| ID | 碟數 | 碟組合 | 權重（平行仲裁調校） | 驗證 |
|----|------|--------|----------------------|------|
| S1 | 1 | A | [1] | 基準：單碟滿速、計數器 = 100% |
| S2 | 2 | A+B | 8:1 | 分布精確 + 容量 = A+B |
| S3 | 3 | A+B+C | 8:1:2 | 分布精確 + 容量 = Σ |
| S4 | 4 | A+B+C+D | 8:1:2:1 | 分布精確 + 完整性 + 容量 = Σ |
...
> 2026-08-11 實測。權重依平行仲裁測速（A=2720 / B=407 / C=489 / D=324 MB/s）取整為 **8:1:2:1**。
> 分布驗證：8G 寫入後每碟 wr bytes 對應 chunk 數 = 權重比例，**與確定性映射的預測值完全一致（0 誤差）**；
> 非完整 stripe 的餘量 chunk 依映射落入前段碟，計數與預期吻合。
...
**結論**：
- **疊碟可擴展**：寫入吞吐隨碟數上升 2083 → 2587 → 2788 → 2972 MB/s；容量由 `seg0_end` 宣告（100G carve 驗證）。
- **分布精確**：計數器與宣告權重完全一致（確定性 O(1) 映射，每 byte 恰落一碟）。
- **資料完整**：crc32c 8G 寫入 + verify 0 mismatch；`err=0`。

使用 conf：`/home/yu/tv_s1.conf`、`tv_s2.conf`、`tv_s3.conf`、`tv_s4.conf`（disk0=nvme1n1, disk1=nvme0n1, disk2=sdc, disk3=sdb）。

...
| **8/14 晚第二批（USB 計畫更新版：6.4.2 三表 + F12 曲線）** | **完成**（2026-08-14 晚，三表：S4 vs LVM vs 單碟A 於 4K randwrite/randread/1M 混寫；F12：S4 1M 順序寫 SLC 衰減曲線**平坦無衰減**（A-bound 封頂 3350）；附帶發現 D 閒置時 borrow ON 淨損 24–29%、A1 2517 為 borrow-on config） | `docs/RESULTS.md` + `docs/data/slc_curve_{20g,100g}.txt` |

...
> **2026-08-14 晚定案**：P3 Plus 回 PCIe2.0 x1（solo ~411）；WD Blue 回池（資料池 4 碟）；
> 現役權重（**8/14 晚 B@x4 + D 重加權**，基於 8/14 下午 B@x4 實證）S2 **[37:27]**（寫 3547）、S3 **[64:30:10]**（DMI-aware 3370）、S4 **[64:27:9:4]**（排水態 2517/4292）、mirror **[37:27]**→C。
...
| **8/14 晚第二批（USB 計畫更新版：6.4.2 三表 + F12 曲線）** | **完成**（2026-08-14 晚，三表：S4 vs LVM vs 單碟A 於 4K randwrite/randread/1M 混寫；F12：S4 1M 順序寫 SLC 衰減曲線**平坦無衰減**（A-bound 封頂 3350）；附帶發現 D 閒置時 borrow ON 淨損 16%（4G）至 29%（8G）、A1 2517 為 borrow-on config） | `docs/RESULTS.md` + `docs/data/slc_curve_{20g,100g}.txt` |

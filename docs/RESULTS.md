...
- **stable by-id 綁定**（config `diskN_name` 一律用此，避免 `/dev` 代號重啟變動）：
  ...
  - D = `/dev/disk/by-id/ata-WDC_WDS250G2B0A_201965800292`
  - E = `/dev/disk/by-id/ata-CT250BX100SSD1_1452F001259B`（未入 config）
...
## 8/14 冷態回歸（重啟後 · 現役 6:1:1:1 + auto-weight S4 對照）
...
### 8/14 全卷自動加權定案（auto_weight v2 · 窮舉式搜尋）
...
14. **權重-速比匹配＋窮舉搜尋逼近原生總和（8/14 定案）**：最慢碟權重釘在 1 → 捨入誤差使次慢碟過載（9:2:2:1 卡 2888）；auto_weight v2 窮舉最佳化產出全卷比例權重（S2 **10:2**、S3 **20:4:5**、S4 **56:11:14:6**，模型 2468/2982/3194 = Σsolo 99.9%/99.8%/99.6%）→ 寫入曲線嚴格單調 **2078 < 2410 < 2896 < 3006**，S4 對舊 6:1:1:1（1983）**+52%**。chunk 大小（1M vs 256K）對順序吞吐中性。

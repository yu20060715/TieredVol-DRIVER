...
> **現況拓撲（2026-08-14 下午定案）**：資料池 3 碟 = A（SN750，CPU 直連 PCIe3.0 x4）+ B（P3 Plus，PCH PCIe2.0 **x4**，已解鎖）+ C（MX500，SATA）；**D（WD Blue）已移出資料池**。現役 config 見 [CONFIG.md](CONFIG.md)。
> **碟位/插槽變更紀錄**：8/12–8/13 多次 nvme 代號交換（config 以 by-id 綁碟身分、不受影響）；**8/14 下午 P3 Plus 由 PCIe2.0 x1 換至 PCIe2.0 x4 插槽（solo 415→1522，+3.7x）**，同時移出 WD Blue、舊 config 全數搬至 `/home/yu/backup-20260814-old/`。
> 各歷史章節標註當日拓撲；最終驗收以「8/14 兩組實驗」（見下）為準。
...
A    = nvme1n1      WD SN750 WDS500G3X0C      500G   PCIe 3.0 x4   8.0 GT/s    2064-2080 MB/s
B    = nvme0n1      P3 Plus CT1000P3PSSD8     1T     PCIe 2.0 x4   5.0 GT/s    1517-1522 MB/s（x1 時 389-415）
C    = sdc          MX500 CT500MX500SSD1     500G   SATA 6G       -           516-518 MB/s
D    = sdb          WD Blue WDS250G2B0A      250G   SATA 6G       -           220-267 MB/s（8/14 已移出資料池）
...
- **stable by-id 綁定**（config `diskN_name` 一律用此，避免 `/dev` 代號重啟變動）：
  ...
  - D = `/dev/disk/by-id/ata-WDC_WDS250G2B0A_201965800292`（已移出資料池）
...
- **插槽角色（2026-08-14 下午）**：A（WD SN750）在 **CPU 直連** PCIe3.0 x4（繞過 DMI）；B（P3 Plus）在 **PCH** PCIe2.0 x4（與 C 共用 DMI 上行）；C（MX500）SATA 在 PCH。
...
```
solo 基線（2026-08-14 下午，B@x4）：A=2064-2080  B=1517-1522  C=516-518 (MB/s)
```
...
| S2 | 2 (A+B) | 6:1 | 2661 | 2786 | A=7022 B=1170（精確） | - |
...
| ID | 推算（權重和=stripe，chunk=1M） | 計數器 | 一致 |
|----|------|--------|------|
| S2 | 8192M = 1170 完整 stripe(7M) + 2M 餘 → A=1170×6+2=7022, B=1170 | 見下方實際 | ✓ |
| S3 | 8192M = 1024 完整 stripe(8M) + 0M 餘 → A=6144, B=1024, C=1024 | 見下方實際 | ✓ |
| S4 | 8192M = 910 完整 stripe(9M) + 2M 餘 → A=910×6+2=5462, B=910, C=910, D=910 | 見下方實際 | ✓ |
| CAP | 1024M = 113 完整 stripe(9M) + 7M 餘 → A=113×6+6=684, B=113+1=114, C=113, D=113 | 見下方實際 | ✓ |

> **2026-08-12 實測計數器**（dmsetup status，bytes）：
> - S2: `A=7363100672 B=1226833920` → 7022:1170 = **6:1** ✓
...
> 餘量 chunk 依權重順序從 A 起連續分配：CAP 末 7M 餘 → A 6M + B 1M（實測 684:114:113:113 吻合）；
> S2 末 2M 餘全落 A（7022 = 7020+2）。確定性行為。
...
現役 conf（8/14 下午定案）：`/etc/tieredvol/tv_s1.conf` [1]、`tv_s2.conf` [37:27]、`tv_s3.conf` [64:30:10]、`tv_mir.conf` [37:27]+mirror=C（對應 repo `configs/`；`tv_s4.conf` 已移除）。
（歷史碟位：8/13 午 disk0=nvme1n1、8/13 晚回到 disk0=nvme0n1；8/14 下午現況 A=SN750=nvme1n1（CPU 直連）、B=P3 Plus=nvme0n1（PCH x4）、C=MX500=sdc。config 一律用 by-id，與 nvme 編號無關。）

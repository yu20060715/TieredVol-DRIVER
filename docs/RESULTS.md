...
2. **S4 舊權重（6:1:1:1）→ 自動加權：1762 → 3006（+70.6%）**，達瓶頸模型 94%、Σsolo 3208 的 93.7%，四碟同時接近飽和。
...
- **讀取**：嚴格單調 3112 < 3899 < 4323（讀不受 DMI 寫入上限影響，S3 三碟並讀全速）。
...
14. **權重-速比匹配＋窮舉搜尋逼近原生總和（8/14 早定案）**：最慢碟權重釘在 1 → 捨入誤差使次慢碟過載（9:2:2:1 卡 2888）；auto_weight v2 窮舉最佳化產出全卷比例權重（S2 **10:2**、S3 **20:4:5**、S4 **56:11:14:6**，模型 2468/2982/3194 = Σsolo 99.9%/99.8%/99.6%）→ 寫入曲線嚴格單調 **2078 < 2410 < 2896 < 3006**，S4 對舊 6:1:1:1（1762）**+70.6%**。chunk 大小（1M vs 256K）對順序吞吐中性。
...
- **B4 補充（8/14 晚第二批）**：上述「借調中性」僅在 **D 飽和背景**下成立。**D 閒置時 borrow ON 反而淨損 16%（4G）至 29%（8G）**：m_s4b（borrow ON，area 2048MB@A、watermark 256KB）1M 寫 4G=2769、8G=2344（8G 時 2G 表滿 + unborrow 回寫更慢），vs 同權重 borrow OFF ~3300。原因：D 份額寫入被重導到 A 的 borrow area，加重 A 瓶頸（A 由 64/104 變 ~68/104 負載）。**A1 排水態 2517 即 borrow-on config 所致**。生產 tv_s4 borrow off 是正確選擇。
